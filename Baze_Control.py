...
def start():
    try:
        ...
        cur=conn.cursor()
        cur.execute("CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, user_iD INTEGER NOT NULL UNIQUE, join_date DATETIME NOT NULL DEFAULT( (DATETIME('now')) ))")
        conn.commit()
        cur.execute("CREATE TABLE IF NOT EXISTS lern (id INTEGER PRIMARY KEY AUTOINCREMENT, user_iD INTEGER NOT NULL, add_time DATETIME NOT NULL DEFAULT((   DATETIME('now'))), progress INTEGER, type TEXT, matereal TEXT)")
        conn.commit()
        cur.execute("CREATE TABLE IF NOT EXISTS lerned (id INTEGER PRIMARY KEY AUTOINCREMENT, user_iD INTEGER NOT NULL, add_time DATETIME NOT NULL DEFAULT((DATETIME('now'))), type TEXT, matereal TEXT)")
        conn.commit()
        cur.execute("CREATE TABLE IF NOT EXISTS archive (id INTEGER PRIMARY KEY AUTOINCREMENT, user_iD INTEGER NOT NULL , type TEXT, matereal TEXT)")
        conn.commit()
        #print("Work 2")
...
def add_lern(user_id, type, matereal, progress_mat):
    try:
        ...
        cur.execute("INSERT INTO 'lern' ('user_id', 'progress', 'type', 'matereal') VALUES(?,?,?,?)", (id_user, progress_mat, type, matereal))
        conn.commit()
    except sqlite3.Error as er:
